...
namespace WoLightning.WoL_Plugin.Game.Rules.Social
{
    ...
    {
        public override string Name { get; } = "Forget to say a Word in a Context";

        public override string Description { get; } = "Triggers whenever you do not say a word in a specific sentence context.";
        ...
        public List<SpecificWordWithContext> ContextualWords { get; set; } = new();
        ...
        [JsonConstructor]
        public DontSayWordInContext() { }
        public DontSayWordInContext(Plugin plugin) : base(plugin)
        {
        ...
        private void Check(XivChatType type, int timestamp, ref SeString senderE, ref SeString messageE, ref bool isHandled)
        {
            try
            {
                if (ContextualWords.Count == 0) return;
                    ...
                    || type == XivChatType.TellOutgoing)) // Tell is a very weird channel. It considers only the target player and gives us no reference of the actual sender.
                {
                    ...
                    foreach (var contextualWord in ContextualWords) // Go through every banned word the user put in.
                    {

                        Logger.Log(3, $"{Name} | Comparing " + message + " against [" + contextualWord + "] which is " + contextualWord.MatchesContext(message));

                        if (contextualWord.MatchesContext(message)) // Now, with both parts. Check each said word, against all banned words. If any of them match, Trigger the Rule and end the Logic.
                        {
                            found = true;
                            break;
                        }
